...
딕셔너리를 이용해서 빠른 검색 가능하도록 만드는게 좋음
```

## 미니 RPG

```
유니티 자체에서 제공하는 네비게이션과 네비게이션 컴포넌트 둘 다 이용
가끔씩 Attack 후 Idle상태에서 헛도는 현상이 있음...
애니메이션은 그냥 CrossFade와 State관리로 하는게 편하다
체력 게이지 바 같은 worldspace UI들은 카메라의 각도에 맞춰주면 편하다
공용되는 부분이 많아지면 부모 클래스를 만들자
Destroy는 정말 조심스러워야 한다
```
